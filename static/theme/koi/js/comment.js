$(function() {
	...
	function generate_comment(comment) {
		...
		html.push('</div></div><p class="reply"><a class="comment-reply-link" href="#respond">reply</a></p></li>');
		return html.join("");
	}
	...
	if (allow_comment) {
		$('#comments').find('a').click(function() {$comment.focus();}); // locate to textarea
	}
	...
	$window.scroll(function(){ // load comments when scroll to the bottom of the page
		if (!complete && !loading && ($document.height() - $window.scrollTop() - $window.height() < 50)) {
			loading = true;
			get_comment();
		}
	});
