$(function() {
	...
	function generate_comment(comment) {
		...
		if (is_admin) {
			html += ' <a href="' + comment_delete_url + comment.id + '/">[Delete]</a>';
		}
		html += ' <a href="#respond" class="comment-reply-link">[Reply]</a></div><div>' + comment.content + '</div></li>';
		return html;
	}
	...
	$load_button.click(function() {
		...
		$.getJSON(url, function(json) {
			...
			if (!next_cursor) {
				...
				$load_button.text('Load next 10 comments');
			}
