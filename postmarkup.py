...
def quote_path(url, coding='utf-8'):
	return quote(url.encode(coding), '_.-+/=~,;&:!*$()?#') if url else ''

def quote_part(url, coding='utf-8'):
	return quote(url.encode(coding), '_.-+=~,;&:!*$()?#') if url else ''
...
def create(include=None, exclude=None, **kwargs):
	...
	add_tag(SimpleTag, 'sub', 'sub')
	add_tag(SimpleTag, 'sup', 'sup')
	add_tag(SimpleTag, 'del', 'del')
	add_tag(SimpleTag, 'ins', 'ins')

	add_tag(TableTag, 'table')
	add_tag(SimpleTag, 'tr', 'tr', inline=False, strip_first_newline=True)
	add_tag(TdTag, 'td')
	add_tag(ThTag, 'th')

	add_tag(LinkTag, 'url', **kwargs)
	add_tag(EmailTag, 'email', **kwargs)
	add_tag(ImgTag, u'img')
	add_tag(FlashTag, u'flash')
	add_tag(ListTag, u'list')
	add_tag(ListItemTag, u'*')

	add_tag(SizeTag, u"size")
	add_tag(ColorTag, u"color")
	add_tag(BackgroundColorTag, u"bcolor")
	add_tag(BackgroundColorTag, u"bgcolor")
...
class SimpleTag(TagBase):
	...
	def __init__(self, name, html_name, inline=True, **kwargs):
		""" html_name -- the html tag to substitute."""
		TagBase.__init__(self, name, inline=inline, **kwargs)
...
class TableTag(TagBase):
	...
	def render_open(self, parser, node_index):
		...
		if self.params:
			width = self.params[:-1] if self.params[-1] == '%' else self.params
			if width.isdigit():
				width = int(width)
				if 0 < width <= 100:
					return u'<table style="width:%s%%">' % width
		return u'<table>'
...
class ThTag(TagBase):
	...
	def render_open(self, parser, node_index):
		...
		if length == 1:
			colspan = params[0]
			if colspan.isdigit() and colspan != '1':
				return u'<th colspan="%s">' % colspan
		elif length == 2:
			colspan = params[0]
			rowspan = params[1]
			if colspan.isdigit() and rowspan.isdigit():
				tag = u'<th'
				if colspan != '1':
					tag += ' colspan="%s"' % colspan
				if rowspan != '1':
					tag += ' rowspan="%s"' % rowspan
				return tag + '>'
		return u'<th>'
...
class LinkTag(TagBase):
	...
	def render_open(self, parser, node_index):
		...
		if '%' in url:
			self.url = escape(url, True)
		else:
			self.url = escape(quoted_url(url))
...
class EmailTag(TagBase):
	...
	def render_open(self, parser, node_index):
		...
		if self.params:
			email = self.params.strip()
		else:
			email = self.get_contents_text(parser).strip()
		...
		return u'<a href="mailto:%s">' % self.email
...
class BlockquoteTag(TagBase):
	...
	def render_open(self, parser, node_index):
		if self.params:
			return u'<blockquote site="%s">' % PostMarkup.standard_replace(self.params)
		else:
			return u'<blockquote>'
...
class QuoteTag(TagBase):
	...
	def render_open(self, parser, node_index):
		if self.params:
			return u'<q site="%s">' % PostMarkup.standard_replace(self.params)
		else:
			return u'<q>'
...
class AlignTag(TagBase):
	...
	def render_open(self, parser, node_index):
		align = self.params.lower()
		if align in ('center', 'right', 'left'):
			align = ' style="text-align:%s"' % align
		else:
			align = ''
		return '<div%s>' % align
...
class FloatTag(TagBase):
	...
	def render_open(self, parser, node_index):
		float = self.params.lower()
		if float in ('right', 'left'):
			float = ' style="float:%s"' % float
		else:
			float = ''
		return '<div%s>' % float
...
class ImgTag(TagBase):
	...
	def render_open(self, parser, node_index):
		...
		return u'<img src="%s"/>' % (PostMarkup.standard_replace_no_break(contents))


class FlashTag(TagBase):
	...
	def render_open(self, parser, node_index):
		...
		if len(params) == 2 and params[0].isdigit() and params[1].isdigit():
			width = min(int(params[0]), 800)
			height = min(int(params[1]), 600)
		else:
			width = 480
			height = 400

		return u'<object type="application/x-shockwave-flash" data="%s" width="%s" height="%s"><param name="movie" value="%s"/><param name="allowScriptAccess" value="sameDomain"/></object>' % (contents, width, height, contents)
...
class ListTag(TagBase):
	...
	def render_open(self, parser, node_index):
		...
		if self.params.isdigit():
			...
			return u'<ol><li>' if self.params == '1' else u'<ol start="%s"><li>' % self.params
		elif self.params == "a":
			self.close_tag = u"</li></ol>"
...
class BackgroundColorTag(TagBase):
	...
	def render_open(self, parser, node_index):
		...
		color = "".join([c for c in color if c in valid_chars])

		if not color:
			self.close_tag = u''
			return

		if len(params) == 2 and params[1] == 'div':
			self.close_tag = u'</div>'
			return u'<div style="background-color:%s">' % color
		else:
			self.close_tag = u'</span>'
			return u'<span style="background-color:%s">' % color
...
class PostMarkup(object):
	...
	line_break_replace = MultiReplace({ u'\n':u'<br/>'})
	...
	def tokenize(cls, post):
		...
		while True:
			...
			next_end_tag_pos = post_find(u']', end_pos)
			if next_end_tag_pos == -1:
				yield TOKEN_TEXT, post[pos:], pos, len(post)
				return
			else:
				next_open_tag_pos = post.rfind(u'[', 0, next_end_tag_pos)
				if next_open_tag_pos > pos:
					yield TOKEN_TEXT, post[pos:next_open_tag_pos], pos, next_open_tag_pos
					end_pos = next_open_tag_pos
					pos = end_pos
					continue
		...
		for tag_type, tag_token, start_pos, end_pos in self.tokenize(post_markup):
			...
			if tag_type == TOKEN_TEXT:
				...
				if escape:
					...
					nodes.append(self.line_break_replace(tag_token))
				continue
			...
			if not end_tag:
				...
				if tag is None:
					if escape:
						nodes.append('[%s]' % self.standard_replace(tag_token))
					else:
						nodes.append('[%s]' % self.line_break_replace(tag_token))
					continue
